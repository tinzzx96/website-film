<?php
require_once '../../config/config.php';
require_once '../../config/tmdb_config.php';

header('Content-Type: application/json');

if (!isset($_GET['query']) || empty($_GET['query'])) {
    echo json_encode([]);
    exit;
}

$query = urlencode($_GET['query']);

try {
    // Search dari TMDb
    $data = fetchTMDb("/search/movie?query={$query}&page=1");
    
    if (!$data || !isset($data['results'])) {
        throw new Exception('Failed to search from TMDb API');
    }
    
    // Convert ke format website
    $result = [];
    foreach ($data['results'] as $movie) {
        $result[] = convertTMDbMovie($movie);
    }
    
    echo json_encode($result, JSON_UNESCAPED_SLASHES);
    
} catch (Exception $e) {
    error_log("Error in search_movies.php: " . $e->getMessage());
    
    // Fallback ke database jika API error
    $searchTerm = $conn->real_escape_string($_GET['query']);
    $sql = "SELECT * FROM movies WHERE title LIKE '%{$searchTerm}%' ORDER BY rating DESC LIMIT 20";
    $dbResult = $conn->query($sql);
    
    $fallbackMovies = [];
    if ($dbResult && $dbResult->num_rows > 0) {
        while($row = $dbResult->fetch_assoc()) {
            $genreArray = json_decode($row['genre'], true);
            if (!is_array($genreArray)) {
                $genreArray = array_map('trim', explode(',', $row['genre']));
            }
            
            $fallbackMovies[] = [
                'id' => (int)$row['id'],
                'title' => $row['title'],
                'rating' => (float)$row['rating'],
                'poster' => $row['poster'],
                'trailer' => $row['trailer'],
                'watchLink' => $row['watchLink'] ?? '',
                'year' => $row['year'],
                'duration' => $row['duration'],
                'genre' => $genreArray,
                'director' => $row['director'] ?? '',
                'plot' => $row['plot'] ?? ''
            ];
        }
    }
    
    echo json_encode($fallbackMovies, JSON_UNESCAPED_SLASHES);
}
?>